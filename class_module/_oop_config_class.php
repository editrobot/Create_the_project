<?php
	class _oop_config_class{
		...
		public function main_run(){
			...
			//创建全局公共js文件
			file_put_contents($this->config_array["project_js_path"].'common.js',
								file_get_contents('js/common.js'),
								LOCK_EX);

			//创建全局公共css文件
			file_put_contents($this->config_array["project_css_path"].'common.css',
								file_get_contents('css/common.css'),
								LOCK_EX);

			//创建全局公共页头文件
			file_put_contents($this->config_array["project_template_path"].'CommonLib.php',
								'<script src="package/jquery-1.12.3.min.js"></script>',
								LOCK_EX);

			//创建全局公共页头文件
			file_put_contents($this->config_array["project_template_path"].'CommonHead.php',
								'',
								LOCK_EX);

			//创建全局公共页脚文件
			file_put_contents($this->config_array["project_template_path"].'CommonFoot.php',
								'',
								LOCK_EX);

			//打包公共库
			$this->zip_class = new _oop_zip_class('package.zip','package/');
			$this->zip_class->files_zip();
			$this->zip_class->files_unzip('package.zip',$this->config_array["project_name"].'/');
			...
			foreach ($this->page_list_array as &$value) {
				...
				//创建css文件
				file_put_contents($this->config_array["project_css_path"].$value->name.'.css',
									'',
									LOCK_EX);
				//创建js文件
				file_put_contents($this->config_array["project_js_path"].$value->name.'.js',
...
console.log(123);
			...
			//创建说明文档
			file_put_contents($this->config_array["project_path"].'README.html',
