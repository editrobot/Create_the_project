...
class _oop_readme_class{
	...
	public function push_page_info($page_class){
		...
		array_push($this->page_list_array,'<br/>'.$this->printf_word("page_file_name").':'.$page_class->name.'.php'.
		'<br/>'.$this->printf_word("page_file_title").':'.$page_class->title.
		'<br/><a href="'.$page_class->name.'.php'.'" target="_blank">链接</a>'
		);
	}

	public function push_module_info($module_class){
		array_push($this->module_list_array,'<br/>'.$this->printf_word("class_name").':'.$module_class->class_name.'.php'
		);
	}
	...
	public function out_put(){
		...
		'<body>'.
		'<h1>'.$this->printf_word("the_document").'</h1>'.
		implode('<br/>',$this->page_list_array).
		'<br/>'.
		implode('<br/>',$this->module_list_array).
		'</body>';
	}
}
?>
