...
$require_array = file_list_in_folder('folder|class_module|category|php');
$require_array_count = 1;
while(isset($require_array[$require_array_count]))
{
	$require_array[$require_array_count] = 'require("class_module/'.$require_array[$require_array_count].'");';
	++$require_array_count;
}
unset($require_array[0]);
file_put_contents('include.php','<?php '.implode("",$require_array).' ?>',LOCK_EX);
echo implode('
',$require_array);
echo 'ok!';
?>